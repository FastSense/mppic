...
#include <iostream>
                ...
                for(tinyxml2::XMLElement* e = grid->FirstChildElement(); e != NULL && row_count < height; e = e->NextSiblingElement(), row_count++)
                  {
                    ...
                    std::stringstream converter;
                    converter << row_str;
                    for(int64_t col_count = width-1; col_count >= 0; col_count--)
                    {
                        float z;
                        converter >> z; 
    ...
    int main(int argc, char *argv[])
    {
        ...
        for(size_t task = 0; task < number_of_tasks; task++)
        {
			...
			std::ofstream myfile;
			std::string res_file_name = "result_task_" + std::to_string(task) + ".txt";
        ...
        auto control = optimizer.evalNextBestControl(init_robot_pose, init_robot_vel, reference_path);
				...
				std::cout << "Time difference = " << static_cast<float>(std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count()) / 1e6   << "[s]" << std::endl;
